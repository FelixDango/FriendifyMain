    ...
    public class UpdateMapper : Profile
    {
        public UpdateMapper()
        {
            CreateMap<UpdateViewModel, User>() // Define the mapping between UpdateViewModel and User models
                .ForMember(dest => dest.UserName, opt => opt.Condition(src => src.Username != null)) // Only map the Username property if it's not null
                .ForMember(dest => dest.UserName, opt => opt.MapFrom(src => src.Username)) // Map the Username property from source to destination
                .ForMember(dest => dest.FirstName, opt => opt.Condition(src => src.FirstName != null)) // Only map the FirstName property if it's not null
                .ForMember(dest => dest.FirstName, opt => opt.MapFrom(src => src.FirstName)) // Map the FirstName property from source to destination
                .ForMember(dest => dest.LastName, opt => opt.Condition(src => src.LastName != null)) // Only map the LastName property if it's not null
                ...
                .ForMember(dest => dest.Email, opt => opt.Condition(src => src.Email != null)) // Only map the Email property if it's not null
                ...
                .ForMember(dest => dest.Biography, opt => opt.Condition(src => src.Biography != null)) // Only map the Biography property if it's not null
